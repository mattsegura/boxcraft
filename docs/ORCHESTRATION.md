...
A **managed session** is a Claude Code instance that boxcraft spawned and controls:
...
If you're running Claude Code in a tmux session called `claude` (the default), boxcraft can observe and prompt it, but it won't appear in the managed sessions list. This is "legacy mode" - it works but isn't part of orchestration.
...
For managed sessions to report events back to boxcraft, Claude Code hooks must be configured globally. The hooks send events to the boxcraft server, which then:
